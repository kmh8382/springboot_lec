...
class B_EntityManagerCRUDTests {
  ...
  void insert_test() {
    ...
    try {
      ...
      // 커밋 (엔티티 매니저가 관리하는 모든 엔티티가 관계형 데이터베이스에 반영합니다. 관계형 데이터베이스에 쿼리를 전송하여 엔티티를 추가합니다.)
      entityTransaction.commit();
      
    } catch (Exception e) {
      // 롤백 (엔티티 매니저가 관리하는 모든 엔티티가 이전 상태로 되돌아갑니다. 롤백이후 다시 begin() 메소드를 호출하여 트랜잭션을 다시 시작할 수 있습니다.)
	...
	void select_test() {
	  ...
	  // 엔티티 매니저를 이용해 엔티티 조회 (조회 시 식별자를 이용하여 조회합니다. 없으면 null 이 반환합니다.)
	 ...
	 @Test
	  void update_test() {
       ...
       // Dirty Checking
       // 영속성 컨텍스트에 저정된 엔티티에 상태가 변경되면, 커밋 시점에 자동으로 해당 변경 내용이 관계형 데이터베이스에 반영되는 것을 의미합니다.
       // 정리) 영속 상태인 엔티티가 커밋 시점에 관계형 데이터베이스에 작용됩니다.
       foundMenu.setMenuName(menuName);
       
       // 커밋 (엔티티의 병경사항이 관계형 데이터베이스에 반영됩니다.)
       entityTransaction.commit();
       ...
       // 수정 확인
       // Assertions.assertThat(entityManager.find(Menu.class, menuCode))...;
   ...
   void delete_test() {
     ...
     try {
       ...
       // 영속성 컨텍스트에 있는 엔티티를 삭제(remove) 상태로 변경
       entityManager.remove(foundMenu);
       
       // 커밋 (관계형 데이터베이스의 쿼리문을 전송하여 엔티티를 삭제합니다.)
       entityTransaction.commit();
