...
@Table(
  ...
  , schema = "db_boot9"   // 스키마(데이터베이스)를 지정
  , uniqueConstraints = { @UniqueConstraint(columnNames = {"user_phone", "create_dt"}) }   // 두개 이상의 칼럼을 묶어서 unique 제약 조건을 지정하는 경우에 필요합니다.
)
...
public class User {
  ...
  @GeneratedValue(      
      ...
      //strategy = GenerationType.AUTO        // 기본키 생성을 데이터베이스에 따라서 자동으로 선택합니다. (MySQL의 경우 시퀀스)
      //strategy = GenerationType.SEQUENCE    // 스퀀스 데이터베이스 객체를 이용합니다 (Oracle)
      
      strategy = GenerationType.TABLE         // 키 값을 생성하는 별도의 테이블을 이용합니다.
    , generator = "user_seq_table_generator"  // @TableGenerator 를 이용해서 키 값을 생성하는 별도의 테이블을
  )
  private int userId;
  ...
  @Column(name = "user_password", nullable = false)   // null 값을 가질수 없습니다. (NOT NULL) 디폴트는 true 입니다.
  ...
  @Column(name = "user_role", length = 100)  // 최대 문자열의 길이가 100 입니다, String 타입에서만 사용할 수 있습니다. 디폴트는 255 입니다.)
