...
class ApplicationTests2 {
  ...
  void 엔티티_프로젝션_test() {
    ...
    // orserable_status -> N 으로 수정해 보세요.
    // Dirty Checking 영속 컨텍스트가 관리하는 엔티티를 수정한 뒤, 커밋하면 변경사항이 데이터베이스에 적용됩니다.    
  ...
  void 스칼라_타입_프로젝션_test() {
    ...
    List<String> categoryName = entityManager.createQuery(jpql, String.class)
                                             .getResultList();
    
    Assertions.assertThat(categoryName).isNotEmpty();
    categoryName.forEach(System.out::println);
    
    // 영속 컨텍스트에 저장되었는지 확인
    // 스칼라 타입 프로젝션은 영속 컨텍스트에 저장되지 않기 때문에 테스트 실패합니다.
    Assertions.assertThat(entityManager.contains(categoryName)).isTrue();
  ...
  void 임베디드_프로젝션_test() {
    ...
    // 임베디드 프로젝션의 결과는 영속 컨ㅌ넥스트에 저장되지 않기 때문에 테스트는 실패합니다.
