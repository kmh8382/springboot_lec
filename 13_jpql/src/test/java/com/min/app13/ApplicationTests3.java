...
class ApplicationTests3 {
  ...
  void 페치_조인_test() {
    ...
    // 1. 최초로 네부 조인 SQL 문이 실행됩니다. 해당 조인 SQL 문에서는 Menu 엔티티 정보만 조회합니다. (1번)
    // 2. 각 Menu 엔팉티의 Category 엔티티의 정보는 나중에 필요할 때(여기서는 출력할 때) 다시 조회합니다.
    // 3. 각 Menu 엔팉티의 Category 엔티티의 정보를 조회할 때 별도의 SQL 문이 최대 Menu 엔티티의 갯수만큼 실행됩니다. (N번)
    
    //String jpql = "SELECT m FROM menu3 m JOIN m.category c";    // 일반 내부 조인
    
    // 페치 조인을 사용하면 최초 조인 SQL 문을 실행할 때 Menu 엔치치와 Category 엔티티 정보를 모두 조회합니다.
